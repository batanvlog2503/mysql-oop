...
const SimilarBlog = () => {
  ...
  const loadPosts = async () => {
    ...
    if (result.status === 200) {
      ...
      alert("Load Similar Post Failed")
    }
  }
  return (
      ...
      <div className="inner-wrap-similar-blogs">
        ...
        <div className="row">
          {similarPost.slice(0, 3).map((post, index) => (
            <div className="view-similar-blog col-lg-4 col-xl-4 col-sm-12 col-12" key = {index}>
              ...
              <p className="similar-blog-category">{post.category.name}</p>
              <div className="similar-blog-social d-flex flex-row justify-content-between">
                <div className="heart-and-message">
                  <span onClick={() => handleClick(1)} className={active === 1 ? "active1" : ""}>
                    <i className="fa-brands fa-gratipay"></i> 13.5K
                  </span>
                  <span  onClick={() => handleClick(2)} className={active === 2 ? "active2" : ""}>
